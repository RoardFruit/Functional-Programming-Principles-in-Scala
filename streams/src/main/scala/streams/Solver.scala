...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val neighbors=initial.flatMap{case (block,move)=> neighborsWithHistory(block,move)}
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter{case (block,move)=>done(block)}
